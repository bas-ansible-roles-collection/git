...
* ensures Git executable is available and a repository can be cloned

The *Local Testing* environment tests multiple scenarios:

1. `test-basic` - ensures Git executable is available a repository can be cloned
...
to bring up a local testing environment
2. `$ cd git/tests/provisioning/site-testing-local`
3. `$ vagrant up`
4. `$ cd ..`
...
Results for these tests can be found [here](https://semaphoreci.com/bas-ansible-roles-collection/git).
